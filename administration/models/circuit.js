...
module.exports.getCircuitGP= function (num,callback) {
    // connection à la base
    db.getConnection(function(err, connexion){
        if(!err){
            // s'il n'y a pas d'erreur de connexion
            // execution de la requête SQL
            let sql = "SELECT grandprix.CIRNUM, CIRNOM FROM circuit JOIN grandprix ON grandprix.CIRNUM=circuit.CIRNUM  WHERE GPNUM="+num;
            //console.log (sql);
            connexion.query(sql, callback);
